...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "tosscoin " + FormatFullVersion();
...
static const char *strMainNetDNSSeed[][2] = {
    {"tosscointools.com", "dnsseed.tosscointools.com"},
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. tosscoin is probably already running."), addrBind.ToString().c_str());
        else
